...
export default function Dashboard() {
  ...
  farmtypes: 'Farm Types',
    ...
    profile: 'My Profile',
    reports: 'Reports',
  'reports_farm': 'Report — Farm',
  'reports_farmers': 'Report — Farmers',
  'reports_visit': 'Report — Visit',
  'reports_advisor': 'Report — Advisor',
    settings: 'Settings',
    roles: 'Roles',
  ...
  reports_farmers: React.createElement(React.lazy(() => import('./reports/ReportFormFarmers')), { reloadKey }),
  reports_visit: React.createElement(React.lazy(() => import('./reports/ReportFormVisit')), { reloadKey }),
  reports_advisor: React.createElement(React.lazy(() => import('./reports/ReportFormAdvisor')), { reloadKey }),
  ...
  const handleOpenTab = (key) => {
    ...
    setTimeout(() => {
      ...
      setTabs(prev => (prev.some(t => norm(t.id) === norm(key)) ? prev : [...prev, { id: key, title: titles[key] || key }]));
  ...
  useEffect(() => {
    ...
    if (tab && titles[tab]) {
      const norm = (s) => (String(s || '').trim().toLowerCase());
      setTabs(prev => (prev.some(t => norm(t.id) === norm(tab)) ? prev : [...prev, { id: tab, title: titles[tab] }]));
      setActiveTabId(tab);
    }
