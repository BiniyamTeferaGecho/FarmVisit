...
export default function ReportFormAdvisor() {
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState(null);
  const [advisors, setAdvisors] = useState([]);
  const [farms, setFarms] = useState([]);
  const [filterField, setFilterField] = useState('AdvisorName');
  ...
  useEffect(() => {
    const fetchLists = async () => {
      try {
        ...
        console.warn('Failed to preload lists for advisor report', err);
      }
    };
    fetchLists();
  }, []);
  ...
  const handleApply = async (filters) => {
    setLoading(true);
    try {
      const res = await api.post('/reports/advisor', filters);
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl font-semibold mb-4">Advisor Report</h2>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:gap-3 gap-2">
        <div className="flex items-center gap-2">
          <select value={filterField} onChange={(e) => setFilterField(e.target.value)} className="px-2 py-1 border rounded">
            {['AdvisorID','AdvisorName','FarmID','Region','CityTown','Phone'].map(f => (
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={async () => { setFilterValue(''); setFilterField('AdvisorName'); await handleApply({}); }} disabled={loading} className="px-3 py-1 bg-gray-100 dark:bg-gray-700 border rounded">Clear</button>
        </div>
      </div>
